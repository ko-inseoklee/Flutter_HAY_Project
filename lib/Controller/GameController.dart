...
class GameController extends BaseController{
  ...
  Future<BalanceGame> loadRandomBalanceGame() async {
    ...
    return new BalanceGame(id: json["id"], category: json["category"].toString(), solution1: json["solution1"].toString(), solution2: json["solution2"].toString());
  }

  Future<StartGame> loadRandomStartGame() async {
    Response response = await dio.get("$baseUrl/$path/start-game/random");
    Map<String,dynamic> json = response.data["data"];
    return new StartGame(id: json["id"], category: json["category"].toString(),question: json["question"].toString(), example: json["example"]);
  }

  Future<WithOneMouth> loadRandomWOMGame() async {
    Response response = await dio.get("$baseUrl/$path/with-one-mouth/random");
    Map<String,dynamic> json = response.data["data"];
    return new WithOneMouth(id: json["id"], category: "초성게임", word2: json["words2"].toString(), word1: json["words1"].toString());
  }
  ...
  Future<OXQuiz> loadRandomOXQuizGame() async {
    ...
    return new OXQuiz(id: json["id"], category: "OX퀴즈", question: json["question"].toString(), answer: answer);
  }

  Future<Quiz> loadRandomQuizGame() async {
    Response response = await dio.get("$baseUrl/$path/quiz/random");
    Map<String,dynamic> json = response.data["data"];
    return new Quiz(id: json["id"], category: "퀴즈", question: json["question"].toString(), answer: json["answer"].toString());
  }
}